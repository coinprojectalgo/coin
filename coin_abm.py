...
class CoinModel:
    ...
    def _random_empty_cell(self) -> Tuple[int, int]:
        if len(self.occupancy) >= self.grid_size * self.grid_size:
    ...
    def _initialize_population(self) -> None:
        if self.num_soldiers == 100 and self.grid_size == 50:
            for i in range(10):
                for j in range(10):
                    x = i * 5 + 2
                    y = j * 5 + 2
    ...
    def summary_figure(self) -> plt.Figure:
        ...
        self.plot_grid(axes[0])
        self.plot_histories(axes[1])
        self.plot_anger_fear(axes[2])
        fig.suptitle(
